...
func (t *ThreadVCIList) VCIMainLoginInit() *fsmvci.VehicleChargingInterfaceLogin  {
	...
	protocMap := protoc.(map[string]interface{})
	return &fsmvci.VehicleChargingInterfaceLogin{
		VehicleChargingProtoVersion: protocMap["VehicleChargingProtoVersion"].(string),
		VehicleChargingVendor:       protocMap["MainStateMachineVendor"].(string),
		SelfCheckRul:                0,
	}
}
...
func (t *ThreadVCIList) VehicleChargingInterfaceLoginAnsInit (GunId uint32, client *udp.Client) *fsmvci.VCIChargingRTpush {
	...
	return &fsmvci.VCIChargingRTpush{
		ID:           &fsmvci.Uint32Value{Value: GunId},
		RTpushCtr:    &fsmvci.Uint32Value{Value: uint32(client.RTpushCtr)},
		GunDesireMsg: nil,
		GunHaltMsg:   nil,
		ReConnect:    nil,
		FaultList:    nil,
...
func (t *ThreadVCIList) buildSonHttp (c *gin.Context) {
	...
	rt := t.VehicleChargingInterfaceLoginAnsInit(VCIGun.Gun, t.SonThread.WorkList[VCIGun.Gun])
	t.SonThread.WorkList[VCIGun.Gun].WriteMsg(0x08, rt)
...
func (t *ThreadVCIList) heartMain (c *gin.Context) {
	...
	var VCImainHeartbeatReq fsmvci.VCImainHeartbeatReq
	Utils.Json2PB(register.MessageBody, &VCImainHeartbeatReq)
	for _, v := range t.MainThread.WorkList {
		VCImainHeartbeatReq.HeartbeatCtr = &fsmvci.Uint32Value{Value: uint32(v.HeartbeatCtr)}
		VCImainHeartbeatReq.CurrentTime = &fsmvci.DateTimeLong{Time: uint64(time.Now().UnixMilli())}
		v.WriteMsg(0x02, &VCImainHeartbeatReq)
	}
...
func (t *ThreadVCIList) getSqlMessage(c *gin.Context) {
	...
	session := database.MySQL.Table("test_message").Where("project_id = ?", sqlMsg.MessageType)
	...
	resqData, err := session.Cols([]string{"id", "message_code", "message_name", "message_body", "project_name", "project_id"}...).QueryString()
	if err != nil {
		c.AbortWithStatusJSON(http.StatusBadRequest, gin.H{
			"code":1,
			"data":err,
